...
FILE=$1
LOG_FILE=/tmp/image-uploader.log

echo "$(date '+%a %d %b %Y %X') - Uploading picture file: $FILE" >> $LOG_FILE
/opt/motion-uploader/uploader.py /opt/motion-uploader/uploader.cfg "$FILE" snap >> $LOG_FILE 2>&1
echo "$(date '+%a %d %b %Y %X') - Upload of picture file: $FILE completed" >> $LOG_FILE
